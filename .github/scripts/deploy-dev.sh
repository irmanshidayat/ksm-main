#!/bin/bash
# =============================================================================
# Deployment Script untuk Development Server
# =============================================================================
...
DEPLOY_PATH="${1:-/opt/ksm-main-dev}"
...
if [ ! -f .env ]; then
  ...
  if [ -f env.dev.example ]; then
    ...
    echo "⚠️  env.dev.example not found. Services may fail without proper configuration."
  fi
fi
  ...
  if docker-compose -f docker-compose.yml up -d 2>&1 | tee /tmp/start.log; then
    ...
    if grep -q "container name.*is already in use" /tmp/start.log || grep -q "Conflict.*container name" /tmp/start.log; then
      ...
      if [ -z "$CONFLICT_CONTAINER" ]; then
        # Try alternative format: "Conflict. The container name \"/KSM-mysql-dev\" is already in use"
        CONFLICT_CONTAINER=$(grep -oE "container name.*\"([^\"]+)\"" /tmp/start.log | grep -oE "/[^\"]+" | tr -d '/' | head -1 || echo "")
      fi
...
SSL_DIR="$DEPLOY_PATH/ksm-main/infrastructure/nginx/ssl"
CERT_FILE="$SSL_DIR/cert.pem"
KEY_FILE="$SSL_DIR/key.pem"

# Also check alternative path (old structure)
if [ ! -f "$CERT_FILE" ] || [ ! -f "$KEY_FILE" ]; then
  ALT_SSL_DIR="$DEPLOY_PATH/infrastructure/nginx/ssl"
  ...
  if [ -f "$ALT_CERT_FILE" ] && [ -f "$ALT_KEY_FILE" ]; then
    ...
    echo "   2. Run: ./scripts/setup-ssl.sh dev your-email@example.com"
